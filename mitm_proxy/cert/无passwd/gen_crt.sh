...
if [ "$operation" = "root" ];then
    ...
    openssl genrsa  -out cert.key $key_length
    openssl req -new -out cert.req -key cert.key -subj $subj
    openssl x509 -req -in cert.req -out cert.cer -CAkey $ca_key -CA $ca_cer -days $expire_days -CAcreateserial -CAserial serial
